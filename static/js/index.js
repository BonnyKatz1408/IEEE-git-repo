...
document.addEventListener('click', (e) => {
  if (!e.target.closest('.graph-node') && !e.target.closest('#demoBtn')) {
    highlightMode = false;
    document.querySelectorAll('.graph-node').forEach(el => el.classList.remove('is-active'));
  }
});

window.pulseBackgroundGraph = function() {
  const activeMods = getActiveModules();
  const target = activeMods.find(m => m.id === "user_service") || activeMods[0];
  if (!target) return;
  highlightMode = target.id;
